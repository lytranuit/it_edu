    ...
    <div class="row clearfix">
        ...
        <Dialog v-model:visible="productDialog" :header="headerForm" :modal="true" class="p-fluid">
            ...
            <div class="row mb-2">
                <div class="field col">
                    ...
                    <input id="name" class="form-control form-control-sm" v-model.trim="model.title" required="true"
                        :class="{ 'p-invalid': submitted && !model.title }" />
                    <small class="p-error" v-if="submitted && !model.title">Required.</small>
                </div>
            </div>
        ...
        <Dialog v-model:visible="deleteProductDialog" header="Xác nhận" :modal="true">
            <div class="confirmation-content">
                ...
                <span v-if="model">Bạn có muốn xóa <b>{{ model.title }}</b> này không?</span>
            </div>
            <template #footer>
...
const showing = ref([]);
const column_cache = "columns_topic"; ////
const first = ref(0);
const rows = ref(10);
const draw = ref(0);
...
const loadLazyData = () => {
    loading.value = true;
    axiosinstance.post("/v1/topic/table", lazyParams.value, {
        headers: {
...
const saveProduct = () => {
    ...
    // }
    axiosinstance.post("/v1/topic/save", model.value, {
        headers: {
            'Content-Type': 'multipart/form-data'
        }
    }).then((res) => {
        ...
        if (res.success) {
            if (model.value.old_key != null) {
                //edit
                toast.add({ severity: 'success', summary: 'Thành công', detail: 'Cập nhật ' + model.value.title + ' thành công', life: 3000 });
            }
            else {
...
const deleteProduct = () => {
    waiting.value = true;
    axiosinstance.post("/v1/topic/remove", { item: [model.value.id] }, {
        headers: {
            'Content-Type': 'multipart/form-data'
        }
    }).then((res) => {
...
const deleteSelectedProducts = () => {
    ...
    waiting.value = true;
    axiosinstance.post("/v1/topic/remove", { item: items }, {
        headers: {
            'Content-Type': 'multipart/form-data'
        }
    }).then((res) => {
