...
const showing = ref([]);
const column_cache = "columns_exams"; ////
const first = ref(0);
const rows = ref(10);
const draw = ref(0);
...
const loadLazyData = () => {
    loading.value = true;
    axiosinstance.post("/v1/exam/table", lazyParams.value, {
        headers: {
...
const deleteProduct = () => {
    waiting.value = true;
    axiosinstance.post("/v1/exam/remove", { item: [model.value.id] }, {
        headers: {
            'Content-Type': 'multipart/form-data'
        }
    }).then((res) => {
...
const deleteSelectedProducts = () => {
    ...
    waiting.value = true;
    axiosinstance.post("/v1/exam/remove", { item: item }, {
        headers: {
            'Content-Type': 'multipart/form-data'
        }
    }).then((res) => {
