    ...
    <div class="row clearfix">
                ...
                <div class="card-body" style="overflow:auto;position: relative;">
                    <DataTable showGridlines :value="datatable" :lazy="true" ref="dt" :paginator="true"
                        ...
                        <Column style="width:1rem">
                            <template #body="slotProps">
                                <a class="p-link text-warning mr-2 font-16" @click="edit(slotProps.data)"><i
        ...
        <Dialog v-model:visible="deleteProductDialog" header="Xác nhận" :modal="true">
            <div class="confirmation-content">
                ...
                <span v-if="model">Bạn có muốn xóa mục này không?</span>
            </div>
            <template #footer>
...
const showing = ref([]);
const column_cache = "columns_questions"; ////
const first = ref(0);
const rows = ref(10);
const draw = ref(0);
...
const loadLazyData = () => {
    loading.value = true;
    axiosinstance.post("/v1/question/table", lazyParams.value, {
        headers: {
...
const deleteProduct = () => {
    waiting.value = true;
    axiosinstance.post("/v1/question/remove", { item: [model.value.id] }, {
        headers: {
            'Content-Type': 'multipart/form-data'
        }
    }).then((res) => {
...
const deleteSelectedProducts = () => {
    ...
    waiting.value = true;
    axiosinstance.post("/v1/question/remove", { item: item }, {
        headers: {
            'Content-Type': 'multipart/form-data'
        }
    }).then((res) => {
...
const edit = (m) => {
    router.push("/question/edit/" + m.id);
}
////Core
onMounted(() => {
